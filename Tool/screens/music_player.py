...
class MusicPlayerScreen(Screen):
        ...
        with Container(id="main-container"):
            with Vertical(id="player-container"):
                ...
                with Container(id="playlist-section", classes="section"):
                    yield Static("📋 Playlist", id="playlist-label", classes="section-title")
                ...
                with Container(id="now-playing-section", classes="section now-playing"):
                    yield Static("🎧 Now Playing", id="now-playing-label", classes="section-title")
                    yield Static("No song selected", id="current-song", classes="current-song")
                    yield Static("00:00 / 00:00", id="time-display", classes="time-display")
                    yield ProgressBar(total=100, id="progress-bar", classes="progress-bar")
        yield Footer()
    ...
    def _get_download_path(self) -> Optional[Path]:
        ...
        try:
            download_path = Path(self.app.settings.settings["download_path"])
    ...
    def _update_playlist_ui(self) -> None:
        ...
        for song in self.songs:
            ...
            playlist.append(static)
        
        if self.songs:
            self.notify(f"Found {len(self.songs)} songs", severity="success")
    ...
    def play_song(self, index: int) -> None:
        ...
        try:
            ...
            if self._start_playback(song, index):
                ...
                self.notify(f"Now playing: {song['title']}", severity="success")
    ...
    def _start_progress_tracking(self) -> None:
        ...
        self.update_timer = self.set_interval(self.PROGRESS_UPDATE_INTERVAL, self.update_progress)
    ...
    def _should_update_progress(self) -> bool:
        ...
        return (self.is_playing and 
                self.current_song_index is not None and 
                self._is_vlc_available())
    ...
    def auto_next(self) -> None:
        ...
        if self._has_next_song():
            self.play_song(self.current_song_index + 1)
        else:
            self.stop()
            self.notify("Reached end of playlist", severity="info")
    
    def _has_next_song(self) -> bool:
        """Check if there's a next song to play"""
        return (self.current_song_index is not None and 
